...
<body class="bg-slate-100 min-h-screen p-6">
    <div class="max-w-4xl mx-auto bg-white rounded-3xl shadow border border-slate-200 p-6">
        <h1 class="text-3xl font-extrabold text-slate-800 mb-2">Tambah Sesi Training</h1>
        <p class="text-slate-500 mb-6">{{ $training->title }}</p>

        <form method="POST" action="{{ route('admin.sessions.store', $training->id) }}" class="space-y-5">
            @csrf

            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-2">Nama Sesi</label>
                <input type="text" name="session_name" class="w-full rounded-2xl border border-slate-300 px-4 py-3">
            </div>

            <div class="grid grid-cols-1 md:grid-cols-4 gap-5">
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Hari</label>
                    <input type="text" name="day_name" class="w-full rounded-2xl border border-slate-300 px-4 py-3" placeholder="Senin">
                </div>

                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Tanggal</label>
                    <input type="date" name="session_date" class="w-full rounded-2xl border border-slate-300 px-4 py-3">
                </div>

                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Jam Mulai</label>
                    <input type="time" name="start_time" class="w-full rounded-2xl border border-slate-300 px-4 py-3">
                </div>

                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Jam Selesai</label>
                    <input type="time" name="end_time" class="w-full rounded-2xl border border-slate-300 px-4 py-3">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div>
                    ...
                    <input type="number" name="quota" min="1" value="30" class="w-full rounded-2xl border border-slate-300 px-4 py-3">
                </div>

                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Quiz Open</label>
                    <input type="datetime-local" name="quiz_open_at" class="w-full rounded-2xl border border-slate-300 px-4 py-3">
                </div>

                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Quiz Close</label>
                    <input type="datetime-local" name="quiz_close_at" class="w-full rounded-2xl border border-slate-300 px-4 py-3">
                </div>
            </div>

            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-2">Link Materi</label>
                <input type="url" name="material_link" class="w-full rounded-2xl border border-slate-300 px-4 py-3" placeholder="https://...">
            </div>

            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-2">Link Diklat / Sertifikat</label>
                <input type="url" name="diklat_link" class="w-full rounded-2xl border border-slate-300 px-4 py-3" placeholder="https://...">
            </div>
